<template>
    <div class="calendar" :style="{marginTop:days.length==6?'0':'20px'}">
        <table cellpadding="5">
        <thead>
            <tr>
                <td v-for="(week, index) in weeks" :key="index" :style="{color:(index==0 || index == 6 ? '#F06151' : '')}"   class="week">{{week}}</td>
            </tr>
        </thead>
        <tbody>
        <tr v-for="(day,k1) in days" :key="k1" style="{'animation-delay',(k1*30)+'ms'}">
            <td v-for="(child,k2) in day" :key="k2" :class="{ 'selected': child.selected, 'disabled': child.disabled }" @click="select(child)">
                <div class="child-timer" style="width: 100%; height: 90%">
                  <div class="time_wrapper" :class="child.currentMonth&&child.day==selectDate?'current_time_wrapper':''">
                      <span ref="selectDate" :class="{'red':k2==0||k2==6||((child.isLunarFestival||child.isGregorianFestival) && lunar)}">{{ child.selected ? child.day : '' }}</span>
                      <div class="text" v-if="child.eventName!=undefined">{{child.eventName}}</div>
                      <span class="text" :style="{ color: k2==0||k2==6||(child.isLunarFestival||child.isGregorianFestival) && lunar ? '#ea6151' : '' }" :class="{'isLunarFestival':child.isLunarFestival,'isGregorianFestival':child.isGregorianFestival }" v-if="lunar">{{ child.selected ? child.lunar : '' }} </span>
                  </div>
                </div>
            </td>
        </tr>
...
//        }
      ...
      rangeBegin: [],
      rangeEnd: [],
      selectDate:new Date().getDate()    //选择的日期
    };
  },
  watch: {
    events() {
  ...
  methods: {
    ...
    render(y, m) {
      ...
      if(temp[temp.length-1][0].day<temp[temp.length-2][0].day){
        temp.length=temp.length-1;
      }
      temp.map((item,key1)=>{
        if(key1!=0&&key1!=temp.length-1){
          item.map((i,key2)=>{
            i.currentMonth=true;
          })
        }else if(key1==0){
          item.map((i,key2)=>{
            i.currentMonth=i.day<=7?true:false;
          })
        }else{
          item.map((i,key2)=>{
            i.currentMonth=i.day>7?true:false;
...
<style lang="less" scoped>
  ...
  .calendar td.selected .child-timer,
  .calendar td.disabled .child-timer {
